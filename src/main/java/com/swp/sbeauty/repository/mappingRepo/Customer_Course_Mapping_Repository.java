...
public interface Customer_Course_Mapping_Repository extends JpaRepository<Customer_Course_Mapping, Long> {
    ...
    @Query(value = "select ccm from Customer_Course_Mapping ccm where ccm.customer_id =?1 and ccm.service_id =?2 and ccm.status ='not yet'")
    public Customer_Course_Mapping getCustomerService(Long customerId, Long serviceId);
